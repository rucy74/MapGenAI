...
namespace MapGenAI.MapGen
{
    ...
    public sealed class MapPlanDescription
    {
        ...
        {
            if(s.type=="passage")return T("폭 ","Dry passage, ")+s.width+T("칸의 마른 통로 — "," cells wide — ")+Name("terrain",s.fill)+", "+Position(s.points[0][0],s.points[0][1])+" → "+Position(s.points.Last()[0],s.points.Last()[1]);
