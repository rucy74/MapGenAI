param([string]$Run='n1',[string]$ProductDll,[string]$ProbeDll,[ValidateSet("open_basin","winding_valley","foothills","followup")][string]$Set="open_basin",[switch]$Graphics)
...
if(-not $ProbeDll){$ProbeDll='C:/Users/choco/Documents/Codex/2026-09-13/new-chat-2/work/mapgenai-guided/repo/tools/natural-landform-probe/bin/Debug/net472/MapGenAI.NaturalProbe.dll'}
$profile=Join-Path $root ('natural-profile-'+$Run)
$output=Join-Path $repo ('docs/analysis/2026-09-23-natural-landforms/native-'+$Run)
...
$arguments=@(('-mapgenAIProbeSet='+$Set),'-batchmode','-nographics',('-savedatafolder="'+$profile+'"'),('-mapgenAIProbe="'+$output+'"'),'-logFile',('"'+(Join-Path $output 'Player.log')+'"'))
