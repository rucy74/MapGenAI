...
최초 프로브는 Root의 WindowStack 생성 전에 창을 열어 실패했다(`before`). 이후 실제 프레임까지 지연해 기존 DLL 오류를 재현했다. 위젯 버튼을 가로채려던 프로브는 메뉴 수명·실행 시점 문제로 실패했고, 초기 GUI Harmony 패치를 일찍 설치한 시도는 시작이 멈춰 소유 마커·실행 인자를 확인한 격리 프로세스만 종료했다(`ui-model-flow-final/harness-timeout.txt`). 해당 패치는 최종 프로브에서 제거했다.
